...
function Index() {
  ...
  const handlePasswordReset = (e: React.ChangeEvent<any>) => {
    e.preventDefault()
    if (!email) {
      setFormValidation({
        valid: false,
  ...
  return (
    <div className='reset'>
      <form className='reset__container' onSubmit={handlePasswordReset}>
        {!formValidation.valid && (
          <Alert severity='error'>{formValidation.validationErroMessage}</Alert>
        )}
        <input
          type='text'
